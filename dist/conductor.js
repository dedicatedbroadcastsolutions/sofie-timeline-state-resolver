...
class Conductor extends eventemitter3_1.EventEmitter {
    ...
    async _resolveTimelineInner() {
        ...
        try {
            ...
            const layersPerDevice = this.filterLayersPerDevice(tlState.layers, Array.from(this.devices.values()).filter((d) => d.initialized === true));
    ...
    _getReplayStateWithCurrentTime(state, now) {
        const filledState = (0, lib_1.fillStateFromDatastore)(state.state, this._datastore);
        ...
        this._actionQueue
            ...
            for (const deviceId of affectedDevices) {
                ...
                const toBeFilled = _.compact([
                    ...
                    [...this._deviceStates[deviceId]].reverse().find((s) => s.time <= now),
